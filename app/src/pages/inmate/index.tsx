...
const InmateHome = () => {
    ...
    function Search(e: React.MouseEvent<HTMLButtonElement>){
        e.preventDefault()
        navigate(`/inmates/search/${queryType.current?.value}/${query.current?.value}`)
    }
    ...
    return (
        ...
        >
            <div 
                className='flex flex-col items-center justify-start w-4/5 px-2 py-4 bg-white rounded-lg shadow-md h-11/12' 
            >
               <img
                    src={logo}
                    alt='logo'
